<x-layouts.app>
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Appointments</h1>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Manage patient appointments</p>
            </div>
            <a href="{{ route('appointments.create') }}" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">Add Appointment</a>
        </div>
                ...
                <tbody>
                    @foreach($appointments as $appointment)
                        <tr class="border-b border-zinc-100 dark:border-zinc-700/50 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700/20 transition">
                            <td class="px-6 py-4 font-medium">{{ $appointment->patient->name }}</td>
                            <td class="px-6 py-4">{{ $appointment->doctor->name }}</td>
                            <td class="px-6 py-4">{{ $appointment->date }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('appointments.edit', $appointment->id) }}" class="rounded-md bg-amber-100 dark:bg-amber-900/30 px-3 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/50 transition">Edit</a>
