<x-layouts.app>
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl p-6">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Create Appointment</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Schedule a new appointment</p>
        </div>

        <div class="max-w-lg rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6">
            <form action="{{ route('appointments.store') }}" method="POST" class="flex flex-col gap-5">
                @csrf
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select name="patient_id" id="patient_id" class="rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" required>
                        <option value="" class="text-zinc-400">Select a patient</option>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select name="doctor_id" id="doctor_id" class="rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" required>
                        <option value="" class="text-zinc-400">Select a doctor</option>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="date" name="date" id="date" class="rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" required>
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="rounded-lg bg-blue-600 px-5 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">Create Appointment</button>
